    ...
    return (
        ...
        <div class="container">
            ...
            <div className='button3'>
                <button className='button1 btn' onClick={updateplease}>Search</button>

            </div>
            <div className='rightinner'>
                <div className='city2'>{initial.location.name}</div>
                <div className='date'>{initial.location.localtime}</div>
                <div className='image'><img src={initial.current.condition.icon} alt="dfdf" /></div>
                <div className='temp'>{initial.current.temp_c}<span>&#176;c</span>
                </div>
                <div className='wind'>Wind: {initial.current.wind_kph}kph</div>
                <div className='humi'>Humidity: {initial.current.humidity}%</div>
                <div className='sky' >Sky:{initial.current.condition.text} <br />U-V:{initial.current.uv}  </div>
                <div className='region'>{initial.location.region}/{initial.location.tz_id}</div>
                <div className='latitudee'>Latitude:{initial.location.lat} <br />Longitude:{initial.location.lon} </div>
